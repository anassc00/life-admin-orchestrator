...
class TestEditTransactionPersistence:
    ...
    def test_should_update_amount_when_provided(self):
        command = EditTransactionCommand(
            transaction_id=self.transaction_id,
            user_id=self.user_id,
            password="correct_password",
        ...
        assert saved_tx.amount == Decimal("250.00")

    def test_should_update_date_when_provided(self):
        new_date = date(2025, 2, 20)
        command = EditTransactionCommand(
            transaction_id=self.transaction_id,
            user_id=self.user_id,
            password="correct_password",
        ...
        saved_tx = self.transaction_repo.save.call_args[0][0]
        assert saved_tx.date == new_date

    def test_should_update_description_when_provided(self):
        command = EditTransactionCommand(
            transaction_id=self.transaction_id,
            user_id=self.user_id,
            password="correct_password",
        ...
        saved_tx = self.transaction_repo.save.call_args[0][0]
        assert saved_tx.description == "Updated description"

    def test_should_update_exchange_rate_when_provided(self):
        command = EditTransactionCommand(
            transaction_id=self.transaction_id,
            user_id=self.user_id,
            password="correct_password",
        ...
        assert result.exchange_rate == Decimal("36.5")
        saved_tx = self.transaction_repo.save.call_args[0][0]
        assert saved_tx.exchange_rate == Decimal("36.5")

    def test_should_update_multiple_fields_at_once(self):
        ...
        assert result.exchange_rate == Decimal("40.0")

    def test_should_preserve_unchanged_fields(self):
        command = EditTransactionCommand(
            transaction_id=self.transaction_id,
            user_id=self.user_id,
            password="correct_password",
        ...
        # Only notes should change
        assert result.notes == "Only updating notes"
